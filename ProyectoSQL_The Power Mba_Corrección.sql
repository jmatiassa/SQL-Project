...
SELECT
    round(variance(replacement_cost)) Variablididad_Coste_Reemplazo
FROM
    film f;
...
forma descendente.*/
    --error al poner un sum en vez de un count en "Numero de alquiler"

SELECT 

rental_date::DATE AS Dia,
...
DESC ;
--comprobación del total de alquileres de la query anterior- 16.044
SELECT
    sum(Numero_alquiler)
    FROM (
    SELECT
...
--Ejercicio 37 en una sola consulta:
SELECT
    (
    SELECT
        actor_id
...
SELECT
    count(r.rental_id) AS "AlquileresRealizados",c.customer_id 
FROM
    customer c
LEFT JOIN rental r ON
    c.customer_id = r.customer_id
--agrupo los alquileres por cliente
...
--SOLO CON UN JOIN
--CONSULTA PREVIA PARA SABER EL CATEGORY ID DE ACTION
SELECT category_id FROM category c WHERE name = 'Action';

-- se añade el category_id en la clausula where
SELECT
    SUM(length)
FROM
    film f
...
ON
    ...
    GROUP BY
        f.title
    HAVING
        count(rental_id) >= 10 )AS pelis_alquiladas;
